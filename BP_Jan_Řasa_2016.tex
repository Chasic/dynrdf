...
\subsection{Shrnutí}
 ...
 \subsubsection{Regulární výraz v placeholderu}
  ...
  stejné řetězce. Příkladem může být třeba URL \url{http://dynrdf.com/object/42}. Pokud by jedna definice používala regulární výraz pouze \uv{obj} a druhá \uv{object}, mohlo
  ...
  Nastavením větší priority pro definici s výrazem \uv{object} dojde ve zmíněném příkladu u URL \url{http://dynrdf.com/object/42} k dřívějšímu porovnání a tedy i 
 ...
 \end{equation}
  ...
  a tudíž k neočekávanému chování celé aplikace. Nestandartní chování může také ovlivnit prostředí, ve kterém je aplikace spuštěna.
  ...
  Ke zdrojovým kódům byly napsány unit a integrační testy, které ověřují správnou funkčnost jednotlivých částí
  ...
  do šablony. Také třída RDFContainer obsahuje několik podobných testů na validaci definic, které se do kontejneru ukládají. 
  
  RDFContainer obsahuje kromě lehčích testů na validaci definic také složitější testy metod, které komunikují převážně s knihovou Apache Jena.
  Jedná se o testy vytváření, aktualizace a mazání definic. Ačkoliv je implementace těchto metod komplikovanější, tak testování probíhá lehce přes dotazy na definici,
  která je testována. Pokud kontejner na dotaz odpoví neočekávaně (například vytvořená definice neexistuje, aktualizovaná definice obsahuje nesprávné atributy, \ldots), je
  tato chyba odchycena a test je neúspěšný.
  
  Další vyšší úrovní testování jsou testy třídy Request. Tato třída obsahuje metody, které ze vstupních parametrů generují výslednou RDF entitu. Jedná se o metody
  ...
  Před samotným testováním se inicializuje kontejner s těmito definicemi. Nad objekty definic jsou poté spouštěny metody pro generování a výsledné
  ...
  Testování API probíhá pro všechny typy požadavků, včetně všech podporovaných serializací. Stejně je tomu i u požadavků klienta na vygenerování RDF entity.
  Ty jsou testovány na všechny testovací definice přes všechny podporované serializace.
  ...
  Tyto tabulky ukazují výsledky testů, u kterých je znázorněna také výhoda používání parametru $group$ v URL adrese požadavku. 
  V prvním případě parametr group je uveden 
  a tudíž není potřeba v nejhorším případě prohledávat celý seznam definic. Pro testovací účely bylo do kontejneru nahráno 300 definic s rozdílnými prioritami a implementován testovací parametr
  do URL adresy, který definoval prioritu testované definice. Tímto byly nasimulovány požadavky na definice s rozdílnou prioritou. V aplikaci JMeter toho bylo
...
TOTAL				&1497		&3596	&98	&5527	&91.4\tabularnewline \hline
 ...
 \end{table}
  ...
  Tento webový server slouží pro dynamické generování RDF entit v několika RDF serializacích. Uživateli je umožněno nadefinovat si vlastní RDF entity a vytvořit si
  k nim
  ...
  Cílem tohoto systému je uživatelům usnadnit tvorbu objektů v RDF formátech pomocí jednotných šablon a zároveň šetřit zdroje za ukládání jejich serializací. 
  Tento server 
  je vhodný například pro časové objekty, kterých může být nekonečně mnoho, nebo pro ty objekty, u kterých se mění pouze pár atributů, které mohou být součásti
  identifikátoru výsledné RDF entity (IRI).
  
  Příkladem takového časového objektu je například interval v rozmezí let. Identifikátorem tohoto objektu může být URL, které obsahuje na konci konkrétní 
  roky - \url{http://obj.com/objects/time-interval/year/2015/2016}. Klientovi je po přístupu na tuto adresu vygenerována výsledná RDF entita, která reprezentuje
  ...
  jinou RDF entitu intervalu v daném rozmezí, přičemž je použita stále stejná šablona.
...
\end{lstlisting}
  ...
  Instalace má tyto kroky:
   ...
   \item Webový archiv, který najdete ve složce \texttt{target}, která se po provedení příkazu vytvořila, nahrajte na aplikační server stejně jako 
